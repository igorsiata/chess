...
GUI::GUI()
    : m_window({800u, 800u}, "Chess"),
      m_squareSize(100), m_clickedSquare(4, 5), m_isPromoting(false),
      m_promotingMove(0) {
  ...
  while (m_window.isOpen()) {
    for (auto event = sf::Event{}; m_window.pollEvent(event);) {
      ...
      if (event.type == sf::Event::KeyPressed && event.key.code == sf::Keyboard::R) {
        // "R" key was just pressed
        if (m_gameInterface.getFullMoveCount() > 0) {
          m_gameInterface.unmakeMove();
        }
      }
    }
    m_window.clear();
...
void GUI::draw_pieces() {
  std::map<Position64, char> allPieces = m_gameInterface.getPiecesAsChar();

  for (auto elem : allPieces) {
    int xPos = (elem.first % 8) * m_squareSize;
    int yPos = (elem.first / 8) * m_squareSize;
